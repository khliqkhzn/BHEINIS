...
// Show only products in the "Kids" category (assuming category_id = 3)
...
<head>
    <meta charset="UTF-8">
    <title>Kids' Products - BHEINIS</title>
    <link rel="stylesheet" href="assets/style.css">
    <style>
        body { font-family: "Segoe UI", sans-serif; background: #f4f7fa; margin: 0; color: #333; }
        header { background: linear-gradient(135deg,#1cc88a,#36b9cc); color: white; padding: 15px 25px; display: flex; justify-content: space-between; align-items: center; }
        h2 { margin: 0; }
        ...
        <tbody>
            <?php if ($result && $result->num_rows > 0): ?>
                <?php while ($row = $result->fetch_assoc()): ?>
                    <tr>
                        <td><?php if (!empty($row['image'])): ?><img src="../uploads/<?= htmlspecialchars($row['image']) ?>" alt="Product Image" style="height: 80px;"><?php else: ?><em>No image</em><?php endif; ?></td>
