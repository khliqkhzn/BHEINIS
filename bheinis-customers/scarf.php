...
// ✅ Log activity: user visited Scarf category
if (isset($_SESSION['user_id'])) {
    $activity_type = "View Category";
    $activity_details = "Visited Scarf category page";
...
<head>
  ...
  <link rel="stylesheet" href="assets2/style2.css" />
    ...
    .color-circles { margin-top: 6px; }
    .color-circles span { display:inline-block; width:15px; height:15px; border-radius:50%; border:1px solid #888; margin-right:4px; vertical-align:middle; }
  </style>
</head>
<body>
...
<div class="top-header">
  <div class="logo-search-container">
    ...
    <!-- ✅ Search Form for Women Category -->
  <form action="search.php" method="GET" class="search-form">
  <input type="hidden" name="category" value="scarf">
  <input type="text" name="q" placeholder="Search Scarf's Products..." required />
...
<nav class="main-nav">
  ...
  <a href="scarf.php" class="active">SCARF</a>
</nav>
  ...
  <div class="products-grid">
    ...
    if ($result && $result->num_rows > 0) {
      while ($product = $result->fetch_assoc()) {
        ...
        echo '<img src="' . $image . '" alt="' . htmlspecialchars($product['name']) . '" style="width:100%; height:220px; object-fit:cover;" />';
        echo '<div class="product-info">';
        echo '<h3>' . htmlspecialchars($product['name']) . '</h3>';
        echo '<p>RM ' . number_format($product['price'], 2) . '</p>';

        // Color circles (if product has color values)
        if (!empty($product['color'])) {
            $colors = explode(",", $product['color']);
            echo '<div class="color-circles">';
            foreach ($colors as $c) {
                $c = trim($c);
                // sanitize color text for title; background uses raw value intentionally (expects css color names)
                echo '<span title="' . htmlspecialchars($c) . '" style="background:' . htmlspecialchars($c) . ';"></span>';
            }
            echo '</div>';
        }
      ...
      echo '<p class="no-products-msg">No products found in Women category.</p>';
    }
    ?>
  </div>
</div>
